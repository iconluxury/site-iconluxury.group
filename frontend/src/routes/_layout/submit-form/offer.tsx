...
const getDisplayValue = (
  cellValue:
    ...
    | null
    | undefined
    | { error?: string; result?: any; text?: string; hyperlink?: string },
): string => {
  ...
  const handleFileChange = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const preview = jsonData.slice(0, MAX_ROWS) as (
          | string
          | number
          | boolean
          | null
    ...
    <Container maxW="container.xl" p={4} bg="white" color="black">
      <VStack spacing={4} align="stretch">
        ...
        <ManualBrandSection
          ...
          isLoading={isLoadingFile}
        />
        <DataTableSection isLoading={isLoadingFile} excelData={excelData} />
    ...
    <ModalContent alignSelf="left" ml={4} mt={16} bg="white" color="black">
      ...
      <ModalBody maxH="60vh" overflowY="auto">
        <Table
          size="sm"
          colorScheme="gray"
          aria-label="Preview rows for header selection"
        >
          <Tbody>
...
const ConfirmHeaderModal: React.FC<ConfirmHeaderModalProps> = ({
  ...
  <Modal isOpen={isOpen} onClose={onClose}>
    ...
    <ModalContent bg="white" color="black">
      ...
      <ModalBody>
        <Text>
          Use row {selectedRowIndex !== null ? selectedRowIndex + 1 : ""} as
          header?
        </Text>
        {selectedRowIndex !== null && (
          <Text mt={2} color="gray.600">
            {previewRows[selectedRowIndex]
              .map((cell) => getDisplayValue(cell))
              .join(", ")}
          </Text>
        )}
      </ModalBody>
      <ModalFooter>
        <Button
          ...
          mr={3}
          onClick={onConfirm}
          aria-label="Confirm header selection"
        >
          Confirm
        </Button>
        <Button
          ...
          onClick={onClose}
          _hover={{ bg: "gray.100" }}
          aria-label="Cancel header confirmation"
        >
          Cancel
        </Button>
      </ModalFooter>
