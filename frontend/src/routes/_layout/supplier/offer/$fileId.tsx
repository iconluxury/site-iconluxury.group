...
import {
  ...
  useQuery,
  useQueryClient,
} from "@tanstack/react-query" // Added useMutation and useQueryClient
...
async function fetchOfferDetails(fileId: string): Promise<OfferDetails> {
  ...
  const response = await fetch(
    `https://external.iconluxury.group/api/luxurymarket/supplier/offers/${fileId}`,
    {
      method: "GET",
      headers: {
...
async function submitOfferForReviewApi(
  fileId: string,
): Promise<{ message: string }> {
  // Expecting a message on success
  const token = getAuthToken()
  const payload = { fileId: Number.parseInt(fileId, 10) } // API likely expects integer fileId

  const response = await fetch(
    "https://external.iconluxury.group/submitOffer",
    {
      method: "POST",
...
const OverviewTab: React.FC<{ offer: OfferDetails }> = ({ offer }) => {
  ...
  if (!offer.status) {
    // Fallback logic if status is not in API response
      ...
      <Table variant="simple" size="md" colorScheme="gray" mb={6}>
        <Tbody>
          ...
          <Tr>
            ...
            <Td>
              {offer.createTime
                ? new Date(offer.createTime).toLocaleString()
                : "N/A"}
            </Td>
          </Tr>
          <Tr>
            <Td fontWeight="semibold" color="gray.700">
              ImageScraperRecords Count
            </Td>
            <Td>{offer.recordCount}</Td>
          </Tr>
          <Tr>
            <Td fontWeight="semibold" color="gray.700">
              NikOfferLoadInitial Count
            </Td>
...
const RecordsTab: React.FC<RecordsTabProps> = ({ offer, searchQuery }) => {
  ...
  const handleSort = (key: keyof NikOfferItem) => {
    // Typed key
    setSortConfig((prev) => {
          ...
          <Table variant="simple" size="sm" colorScheme="gray">
            ...
            <Tbody>
              {displayedRecords.map((record, index) => (
                <Tr
                  key={`${record.fileId}-${record.f0}-${index}`}
                  _hover={{ bg: "gray.50" }}
                >
                  {headers.map((header) => (
                    <Td
                    ...
                    >
                      {String(
                        record[header.key as keyof NikOfferItem] ?? "N/A",
                      )}
                    </Td>
                  ))}
                </Tr>
...
const OfferDetailPage = () => {
  ...
  const searchParams = useSearch({
    from: "/_layout/supplier/offer/$fileId",
  }) as SearchParams
  ...
  const [activeTab, setActiveTab] = useState<number>(
    Number.isNaN(initialTab) || initialTab < 0 || initialTab > 1
      ? 0
      : initialTab,
  )
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      showToast(
        "Success",
        data.message || "Offer submitted for review successfully.",
        "success",
      )
  ...
  if (error && !offerData) {
    // Error and no data (even placeholder)
    return (
      <Container maxW="full" py={6} bg="white">
        <Box textAlign="center" py={10}>
          ...
          <Button
            ...
            colorScheme="blue"
            onClick={() =>
              queryClient.refetchQueries({ queryKey: ["offerDetails", fileId] })
            }
          >
            Retry
          </Button>
  ...
  if (!offerData) {
    // Should ideally be caught by isLoading or error, but as a fallback
    return (
      <Container maxW="full" py={6} bg="white">
  ...
  const tabsConfig = [
    { title: "Overview", component: () => <OverviewTab offer={offerData} /> },
    {
      title: "Records",
      component: () => (
        <RecordsTab offer={offerData} searchQuery={searchQuery} />
      ),
    },
  ]
      ...
      <Box bg="white" shadow="sm">
        ...
        >
          <Box textAlign="left" flex="1">
            ...
            <Text fontSize="sm" color="gray.500">
              Details for supplier offer.{" "}
              {isFetching && isLoading && <Spinner size="xs" ml={2} />}
            </Text>
          </Box>
          <Button
            colorScheme="blue"
        ...
        <TabPanels>
          {tabsConfig.map((tab, index) => (
            <TabPanel key={tab.title} px={{ base: 2, md: 4 }} py={4}>
              ...
              {/* Render only active tab content with isLazy */}
            </TabPanel>
          ))}
        </TabPanels>
      </Tabs>
...
export const Route = createFileRoute("/_layout/supplier/offer/$fileId")({
  ...
  ): { activeTab?: string; search?: string | string[] } => ({
    // Ensure return type matches SearchParams expectation
