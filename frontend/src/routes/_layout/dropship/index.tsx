...
async function fetchJsonStore(
  ...
  try {
    ...
    const message = error.message?.includes("CORS")
      ...
      : error.message ||
        `Network error fetching ${storageType.toUpperCase()} JSON store`
...
async function getSignedUrl(
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/${storageType}/sign?key=${encodeURIComponent(
        key,
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (paths) => {
      ...
      queryClient.setQueryData<S3ListResponse>(
        ...
        (old) => {
          if (!old)
            return { objects: [], totalCount: 0, nextContinuationToken: null }
          return {
            ...old,
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `Successfully deleted ${deletePaths.length} item${
      ...
      >
        <HStack spacing={4} align="center">
          <Text
            fontSize="md"
            fontWeight="medium"
            color={isDragging ? "green.600" : "gray.600"}
          >
