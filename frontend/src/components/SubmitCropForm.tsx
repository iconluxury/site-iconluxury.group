...
const autoMapColumns = (headers: string[]): ColumnMapping => {
  const mapping: ColumnMapping = { style: null, brand: null, image: null }
  const patterns = {
    style: /^(style|product style|style\s*(#|no|number|id)|sku|item\s*(#|no|number))/i,
    brand: /^(brand|manufacturer|make|label|designer|vendor)/i,
  ...
  headers.forEach((header, index) => {
    ...
    else if (patterns.brand.test(h) && mapping.brand === null) mapping.brand = index
    else if (patterns.image.test(h) && mapping.image === null) mapping.image = index
  })
  return mapping
}
...
const SubmitCropForm: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const columnMapping = activeSheet?.columnMapping ?? { style: null, brand: null, image: null }
  ...
  const handleHeaderChange = useCallback(
    (newHeaderIndex: number) => {
      ...
      if (newHeaderIndex < 0 || newHeaderIndex >= activeSheet.rawData.length) return
  ...
  const mappedColumnsForHighlight = useMemo(() => {
    ...
    ;(["style", "brand", "image"] as (keyof ColumnMapping)[]).forEach((k) => {
            ...
            <WrapItem key={sheet.name || index}>
              <Tooltip label={tooltipLabel} placement="top" hasArrow>
                <Button
                  ...
                  cursor="pointer"
                  bg={isActive ? undefined : isComplete ? sheetInactiveBg : sheetWarningHover}
                  _hover={{
                    bg: isActive ? undefined : isComplete ? sheetInactiveHover : sheetWarningHover,
                  }}
    ...
    try {
      for (const [index, sheet] of sheetConfigs.entries()) {
        ...
        if (sheet.manualBrandValue) {
          ...
          formData.append("brandColCrop", indexToColumnLetter(mapping.brand))
        }
      ...
      showToast("Success", `${sheetConfigs.length} crop job(s) submitted successfully`, "success")
          ...
          {step === "map" && (
            <Flex direction={{ base: "column", md: "row" }} gap={4} align="stretch" maxH="70vh" overflow="auto">
              ...
              >
                  ...
                  <Card variant="outline" bg={mappingPanelBg} borderColor={mappingPanelBorder} shadow="xs">
                    <CardBody p={4}>
                      <VStack align="stretch" spacing={3}>
                        ...
                        <Text fontSize="xs" color="subtle">
                          Currently editing: {sheetConfigs[activeSheetIndex]?.name || `Sheet ${activeSheetIndex + 1}`}
                        </Text>
                      </VStack>
                    </CardBody>
                  </Card>
                ...
                {(["style", "image"] as (keyof ColumnMapping)[]).map((field) => (
                  <HStack
                    key={field}
                    gap={2}
                    align="center"
                ...
                <Text fontWeight="bold" mt={4}>Optional</Text>
                <HStack gap={2} align="center" p={2} borderRadius="md">
                  <Text w="180px" fontWeight="semibold">Brand Column:</Text>
                  <Select
                    value={columnMapping.brand !== null ? columnMapping.brand! : ""}
                    onChange={(e) => handleColumnMap(Number(e.target.value), "brand")}
                    placeholder="Unmapped"
                    flex="1"
                  >
                    <option value="">Unmapped</option>
                  ...
                  {columnMapping.brand !== null && (
                    <IconButton
                      aria-label="Clear brand mapping"
                      icon={<CloseIcon />}
                      size="sm"
                      onClick={() => handleClearMapping(columnMapping.brand!)}
                    />
                  )}
                </HStack>
              </VStack>
            ...
            <VStack spacing={4} align="stretch">
              <VStack align="start" spacing={4}>
                ...
                <FormControl isRequired>
                  ...
                  {sendToEmail ? (
                    ...
                    <Text fontSize="sm" color="red.500">
                      No email parameter detected. Add ?sendToEmail=example@domain.com to the iframe URL.
                    </Text>
                  )}
                ...
                <Table variant="simple" size="sm">
                  ...
                  <Tbody>
                    {(["style", "image", "brand"] as (keyof ColumnMapping)[])
                      .filter((key) => columnMapping[key] !== null)
                      .map((key) => (
                        <Tr key={key}>
                          <Td>{key === "image" ? "Image Column (pictures)" : key === "style" ? "Style # Column" : "Brand Column"}</Td>
